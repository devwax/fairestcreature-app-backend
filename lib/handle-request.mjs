...
import { credentialsConfigured, getConfig, secretFingerprint } from './config.mjs';
...
export async function handleRequest(req, res, extras = {}) {
  ...
  if (config.requireAppProxy) {
    ...
    if (!ok) {
      ...
      console.error('[app-proxy] signature failed', diag);
      // Always return safe diagnostics on staging so we can see missing/wrong signature vs secret.
      sendJson(401, {
        status: false,
        data: 'Invalid app proxy signature',
        debug: diag,
      });
      return;
    }
