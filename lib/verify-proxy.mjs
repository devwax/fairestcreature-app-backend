 ...
 * Catch-all routes named `[...path].js` expose the segment as `...path` (with dots).
 */
const IGNORED_QUERY_KEYS = new Set(['path', '...path', 'index']);

function isIgnoredQueryKey(key) {
  if (IGNORED_QUERY_KEYS.has(key)) return true;
  // Defensive: any catch-all style key ending in path (e.g. "..path", "...slug")
...
export function buildAppProxyMessage(query) {
  return Object.entries(query)
    .filter(([key]) => key !== 'signature' && key !== 'hmac' && !isIgnoredQueryKey(key))
...
export function collectProxyQuery(reqUrl, vercelQuery) {
  ...
  for (const [k, v] of Object.entries(vercelQuery)) {
    if (isIgnoredQueryKey(k)) continue;
