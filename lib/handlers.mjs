...
export async function handleCustomerCreate(data) {
  ...
  if (existing) {
    ...
    const updated = await shopify('PUT', `/admin/api/${api}/customers/${existing.id}.json`, {
      customer: {
        id: existing.id,
        first_name: data.first_name || '',
        last_name: data.last_name || '',
    ...
    if (addrResult?.errors) return { status: false, data: addrResult.errors };
  ...
  const created = await shopify('POST', `/admin/api/${api}/customers.json`, payload);
  if (created.errors) return { status: false, data: created.errors };
